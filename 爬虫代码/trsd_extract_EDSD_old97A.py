...
def EDSD_nonote():
    ...
    for line in fr.readlines():
        ...
        #print matcher
        w2 = open(ss+'EDSD_nonote%s.txt'%year,'a')#a代表追加 w代表重写
        if matcher1:
            flag = 1
            w2.write("\n")
            for j in matcher1:
...
def EDSD_note():
    ...
    w2 = open(ss+'EDSD_note%s.txt'%year,'a',encoding='utf-8')#a代表追加 w代表重写
    # temp = ();
    flag = 0
    flag1=0
    for line in fr.readlines():
...
def join():
    ...
    f1= open(ss+'EDSD_note%s.txt'%year)
    f2 =open(ss+'EDSD_nonote%s.txt'%year) 
    ...
    # print(list_note)
    f2_w= open(ss+'EDSD%s.csv'%year,'a')  

...
'''
try:
    os.rename(ss+'EDSD.%s'%year,ss+'EDSD%s.txt'%year)
except:
    pass

p1 = r"^\s{4}..\s([A-Z]{3})\s\s.+\n"#TCC
p2 = r"(^\d{3})\s.\s{2}C\d{3}.+[CM]\s+\d+\n"#010
p3 = r"^\d{3}\s.\s{2}(C\d{3}).+[CM]\s+\d+\n"#C552
p4 = r"^\d{3}\s.\s{2}C\d{3}.+([CM])\s+\d+\n"#M
p5 = r"^\d{3}\s.\s{2}C\d{3}.+[CM]\s+(\d+)\n"#1
p6= r"(^\d{3})\s.\s{2}\d{4}.+[CM]\s{4}\d\s.*\.\.\d+\n|(^\d{3})\s{4}\w\d{3}\s.+[^\d]$\n"#单独的030
p7 =r"^\d{3}\s.\s{2}(\d{4}).+[CM]\s{4}\d\s.*\.\.\d+\n|^\d{3}\s{4}(\w\d{3})\s.+[^\d]$\n"#单独的3286
p8 =r"^\d{3}\s.\s{2}\d{4}.+([CM])\s{4}\d\s.*\.\.\d+\n|^\s{12}[A-Z].+([CM])\s{4}\d\s.*\.\.\d+\n"#单独的M
p9 =r"^\d{3}\s.\s{2}\d{4}.+[CM]\s{4}(\d)\s.*\.\.\d+\n|^\s{12}[A-Z].+[CM]\s{4}(\d)\s.*\.\.\d+\n"#单独的1
p10 =r"^\d{3}\s.\s{2}\d{4}.+[CM]\s{4}\d\s(.*\.\.\d+)\n|^\s{12}[A-Z].+[CM]\s{4}\d\s(.*\.\.\d+)\n"#单独的an..35

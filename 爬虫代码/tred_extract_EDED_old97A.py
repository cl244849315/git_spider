...
def EDED_nonote():
    try:
        ...
        pass
    p1 = r"^.\s\s(\d\d\d\d)\s\s[A-Z].+\n"#匹配1001
    p2 = r"^.\s\s\d\d\d\d\s\s([A-Z].+)\n"
    ...
    for line in fr.readlines():
        ...
        w2 = open(ss+'EDED_nonote%s.txt'%year,'a')#a代表追加 w代表重写
        if matcher1:
            flag = 1
            w2.write("\n")
            for j in matcher1:
...
def EDED_note():
    ...
    w2 = open(ss+'EDED_note%s.txt'%year,'a',encoding='utf-8')#a代表追加 w代表重写
    # temp = ();
    flag = 0
    flag1=0
    for line in fr.readlines():
...
def join():
    ...
    f1= open(ss+'EDED_note%s.txt'%year)
    f2 =open(ss+'EDED_nonote%s.txt'%year) 
    ...
    # print(list_note)
    f2_w= open(ss+'EDED%s.csv'%year,'a')  

...
def IDCD_nonote():
    ...
    p1 = r"^\s{3}..\s(\w\d\d\d)\s.+\n"
    p2 = r"^\s{3}..\s\w\d\d\d\s(.+)\n"
    p3 = r"^\s{6}Desc:\s(.+\.)\n"
    p4 = r"^\s{6}Desc:\s(.+[^\.])\n"
    p5 = r"^\s{12}(.+[^\.])\n"
    p6 = r"^\s{12}(.+\.)\n"
    ...
    for line in fr.readlines():
        ...
        #print matcher
        w2 = open(ss+'IDCD_nonote%s.txt'%year,'a')#a代表追加 w代表重写
        if matcher1:
            flag = 1
            w2.write("\n")
            for j in matcher1:
...
def IDCD_note():
    ...
    w2 = open(ss+'IDCD_note%s.txt'%year,'a',encoding='utf-8')#a代表追加 w代表重写
    # temp = ();
    flag = 0
    flag1=0
    for line in fr.readlines():
...
def join():
    ...
    f1= open(ss+'IDCD_note%s.txt'%year)
    f2 =open(ss+'IDCD_nonote%s.txt'%year) 
    ...
    # print(list_note)
    f2_w= open(ss+'IDCD%s.csv'%year,'a')  

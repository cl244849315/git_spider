...
p2 = r"(^\d{3}).{4}[A-Z]\d{3}.+[CM]\s+.+\n"#010
p3 = r"^\d{3}.{4}([A-Z]\d{3}).+[CM]\s+.+\n"#C552
p4 = r"^\d{3}.{4}[A-Z]\d{3}.+([CM])\s+.+\n"#M
# p5 = r"^\d{3}\s{3}C\d{3}.+[CM]\s+(\d)\n"#1
p6= r"(^\d{3}).{4}\d{4}.+[CM]\s{2}.*(?:.*\.\.\d+|.*\d+)\n"#单独的030
p7 =r"^\d{3}.{4}(\d{4}).+[CM]\s{2}(?:.*\.\.\d+|.*\d+)\n"#单独的3286
p8 =r"^\d{3}.{4}\d{4}.+([CM])\s{2}(?:.*\.\.\d+|.*\d+)\n"#单独的M
# p9 =r"^\d{3}\s{3}\d{4}.+[CM]\s{2}.*\.\.\d+\n"#单独的1
p10 =r"^\d{3}.{4}\d{4}.+[CM]\s{2}.+\s(?:([a-z].*\.\.\d+)|([a-z].*\d+))\n"#单独的an..35
p11=r"(^\d{3}).{4}(\w\d{3})\s(.+[A-Z])\n"#010跨行
p12=r"^\s{12}.+([CM]).+\n"#跨行
...
for line in fr.readlines():
    ...
    matcher11=re.findall(pattern11,line)
    matcher12=re.findall(pattern12,line)
    #print matcher
    ...
    if (matcher11!=[]):
        zz=1
        # print(matcher11)B
        w2.write("\n"+temp+","+matcher11[0][0]+",C000,"+matcher11[0][1])
    if ((matcher12!=[]) and (zz==1)):
        zz=0
        w2.write(","+matcher12[0]+year+",")
    w2.close( )

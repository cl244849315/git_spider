...
year=sy.year#用户自定义
ss="./data/%s/TRCD/"%year#根目录
filename = ss+'TRCD%s.txt'%year#输入文件名
...
'''
try:
    os.rename(ss+'TRCD.%s'%year,ss+'TRCD%s.txt'%year)
except:
    pass

p1 = r"^\s{4}.\s(\w\d{3})\s\s.+\n"#匹配C002
p2 = r"^(\d\d\d)\s{3}\d{4}\s{2}.+[CM]\s+(?:.*\.\.\d+|.*)\n"#C      n3
p3 = r"^\d\d\d\s{3}(\d{4})\s{2}.+[CM]\s+(?:.*\.\.\d+|.*)\n"
p4 = r"^\d\d\d\s{3}\d{4}\s{2}.+([CM])\s+(?:.*\.\.\d+|.*)\n"
p5 = r"^\d\d\d\s{3}\d{4}\s{2}.+[CM]\s+((?:.*\.\.\d+|.*))\n"

p6 = r"^(\d\d\d)\s{3}\d{4}\s{2}.+[^\d]$\n"
p7 = r"^\d\d\d\s{3}(\d{4})\s{2}.+[^\d]$\n"
p8 = r"^\s{13}.+([CM])\s+(?:.*\.\.\d+|.*)\n"
p9 = r"^\s{13}.+[CM]\s+((?:.*\.\.\d+|.*))\n"

...
从11c开始提取
!!!!!!!会出BUG:
0250,SG5,SPS,05A,QALITY,10,C,05A_12302,

'''
import re
import numpy as np
import os
list1 = ['05','06','07','08','09','10','11','12','13','14','15','16','17']

#k1=0
for yearnum in list1:
    for AB in ['A','B']:
        year = yearnum+AB
        if year =='17B':
            break
        print year
        ss="C:/Users/lenovo/Desktop/csv/edmd/%s/edmd/"%year
        # filename=ss+"/EDMDI1.17A"

        def get_tag():
            try:
                os.rename(ss+"EDMDI1.%s"%year,ss+"EDMDI1.txt")
            except:
                pass
        ...
        def trmd_b1_nonote(list_tag):
            ...
            for MM in range(len(list_tag)):
                try:
                    os.rename(ss+'%s_D.'%list_tag[MM]+year,ss+'%s.txt'%list_tag[MM])
                except:
                ...
                #特殊00120 * DTM Date/time/period                         C   1                |
                pattern1   =  re.compile(r"(^\d{4,5})(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+[CM]\s{3}\d*\s{1,}\|{0,}\n")#00010
                pattern1_2 =  re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))([A-Z]{3}).+[CM]\s{3}\d*\s{1,}\|{0,}\n")#UNH
                #pattern1_3 =  re.compile(r"^\d{5}\s{3}[A-Z]{3}(.+)[CM]\s{3}\d*\s{1,}\|{0,}\n")#Message header
                pattern1_4 =  re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+([CM])\s{3}\d*\s{1,}\|{0,}\n")#C
                pattern1_5 =  re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+[CM]\s{3}(\d*)\s{1,}\|{0,}\n")#1
                pattern1_6 =  re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))SPS.+\n")#SPS多行
                ...
                pattern5_1 = re.compile(r"(^\d{4,5})(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+[CM]\s{3}\d*\-+\+{1,10}\|{0,20}\n" )
                pattern5_2 = re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))([A-Z]{3}).+[CM]\s{3}\d*\-+\+{1,10}\|{0,20}\n" )
                pattern5_3 = re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+([CM])\s{3}\d*\-+\+{1,10}\|{0,20}\n" )
                pattern5_4 = re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+[CM]\s{3}(\d*)\-+\+{1,10}\|{0,20}\n" )
                #以下是确定层级关系
                #匹配每组的单独结尾的一行即没有Segment group的以+、+|、+||、+|||……结尾的
                pattern5 = re.compile(r"^\d{4,5}(?:(?:\s{3})|(?:\s\*\s))[A-Z]{3}.+[CM]\s{3}\d*\-+\+\|{0,10}\n" )
                #匹配每组的开头一行即有Segment group的以+、+|、+||、+|||……结尾的
                pattern2_1 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\n" )#+结尾
                pattern2_2 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\n" )#+|结尾
                pattern2_3 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\n" )#+||结尾
                pattern2_4 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\n" )
                pattern2_5 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\|\n" )
                pattern2_6 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\|\|\n" )
                pattern2_7 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\|\|\|\n" )
                pattern2_8 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\|\|\|\|\n" )
                pattern2_9 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\|\|\|\|\|\n" )
                pattern2_10 = re.compile(r"^\d{4,5}.+Segment\sgroup\s(\d*).+[CM]\s{3}\d*\-+\+\|\|\|\|\|\|\|\|\|\n" )
                #匹配有同时多个组同时结束的情况，即以++、++|、++||……++、++|、++||……等结尾的
                pattern3_1 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{2}\|{0,20}\n")# 匹配++、++|、++||……等结尾
                pattern3_2 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{3}\|{0,20}\n")# 匹配+++、+++|、+++||……等结尾
                pattern3_3 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{4}\|{0,20}\n")
                pattern3_4 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{5}\|{0,20}\n")
                pattern3_5 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{6}\|{0,20}\n")
                pattern3_6 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{7}\|{0,20}\n")
                pattern3_7 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{8}\|{0,20}\n")
                pattern3_8 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{9}\|{0,20}\n")
                pattern3_9 = re.compile(r"^\d{4,5}.+[CM]\s{3}\d*\-+\+{10}\|{0,20}\n")
                ...
                for line in fr.readlines():
                    ...
                    matcher1_6 = re.findall(pattern1_6,line)
                    ...
                    matcher2_8 = re.findall(pattern2_8,line)
                    matcher2_9 = re.findall(pattern2_9,line)
                    matcher2_10 = re.findall(pattern2_10,line)
                    ...
                    matcher3_7 = re.findall(pattern3_7,line)
                    matcher3_8 = re.findall(pattern3_8,line)
                    matcher3_9 = re.findall(pattern3_9,line)
                ...
                #                w2.write(k)
                    ...
                    if (matcher1_6!=[]):
                        ...
                        if numgr ==0:
                            ...
                            w2.write("SG"+str(numgr)+",")
                        w2.write("SPS"+year+","+list_tag[MM]+",10,C")
                w2.close()
                fr.close()
        def trmd_b1_note(list_tag):
            ...
            for MM in range(len(list_tag)):
                ...
                for line in fr.readlines():
                    ...
                    pattern_head = re.compile(r".MESSAGE\sDEFINITION\n.")
                    matcher_head = re.findall(pattern_head,line)
                    if (matcher_head!=[]):
                        flag = 20
                    ...
                    for i in range(10):
                        k = list1[i]
                        # print k
                        pattern1 = re.compile(r"^(\d{5})\s{"+str(k)+"}[^ ].+\n")
                        #11A-05A是以4位数字开头
                        pattern1_2 = re.compile(r"^(\d{4})\s{"+str(k)+"}[^ ].+\n")
                        matcher1 = re.findall(pattern1,line)
                        matcher1_2 = re.findall(pattern1_2,line)
                        if (matcher1!=[]):
                            k1=k1+1
                ...
                wopen.close()
        def join(list_tag):
            f2_w= open("C:/Users/lenovo/Desktop/csv/edmd/b1.csv",'a')
            #f2_w.write("code_pos,parent,TRSD_tag,year,code_tag,S,R,bridge01_id,code_desc\n")
            f2_w.close()
            for MM in range(len(list_tag)):
                ...
                f2_w= open("C:/Users/lenovo/Desktop/csv/edmd/b1.csv",'a')
                j=0
                #f2_w.write("code_pos,parent,TRSD_tag,year,code_tag,S,R,code_desc\n")
                for line2 in f2:
                    q2 =q2+1
                    if q2!=1:
                        str11="%s,%s\n"%(list_note[j].strip('\n'),line2.strip('\n'))
            ...
            f2_w.close()
        #def addindex():
        #    f3= open(ss+'new/b1%s.csv'%year,'a')
        #open(ss+'new/b1%s.csv'%year,'a')
